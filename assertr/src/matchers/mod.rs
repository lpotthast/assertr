...
//!
//! A [`PartialMatch`] stays generic over renderers supported by its selected matchers. A
//! [`satisfying`] closure can select a concrete renderer through its
...
//! ```rust
//! use assertr::prelude::*;
//! use assertr::matchers::{Description, MatchContext, MatchResult};
//! struct Even;
//! impl<R> AssertrMatcher<i32, R> for Even {
//!     fn describe(&self, _: &MatchContext<'_, R>) -> Description {
//!         Description::new("satisfies the even-number constraint")
//!     }
...
pub use context::{Description, MatchContext, MatchFork, MatchResult, Mismatch};
...
pub trait AssertrMatcher<Actual: ?Sized, R = DebugRenderer> {
    ...
    fn describe(&self, _context: &MatchContext<'_, R>) -> Description {
        Description::new(type_name::<Self>())
    }
