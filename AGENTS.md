...
and all changes must be merged into that section instead of falsely being added to "## [Unreleased]".

Every changelog item must describe only the net change from the exact immediately preceding release to the current
release candidate. Never infer or describe changes relative to intermediate development states, whether committed or
uncommitted.
...
as a doctest. README examples must therefore compile and pass as top-level snippets in the minimal configuration. The
README is a landing page. Detailed guides belong in the rustdoc of the API item that owns the topic, which the README
links to.

## Writing

Instead of em-dashes or ";" use sentences where they would also suffice.
...
- **`AssertrPartialEq<Rhs>`** (`lib.rs`) - Custom equality trait with `EqContext` for field-by-field difference
  reporting.
- **Rendering** (`renderer/`) - `ValueRenderer` (default `DebugRenderer`) formats leaf values. The public
  `RenderingContext`, obtained through `AssertThat::render()`, offers `value` and `values` to custom assertions and
  composes leaves into variant, struct-field, list, set, map, and entry-list syntax through crate-private adapters. It
  applies the chain's `RenderingBudget`, which derived chains inherit. Every adapter, including the ones `EqContext`
  returns, is a named type without a destructor on purpose: an opaque `impl Debug` would keep the chain or context
  borrowed until the adapter leaves scope and block returning `self` or recording a difference afterwards.
...
- `collection/`, `set/`, `map/` - The generic container families. Capability-based extension traits, no per-type
  assertion traits, so implementations cannot drift apart in bounds or diagnostics:
  - `Collection: HasLength` (`collection/mod.rs`) carries only `Item`, diagnostic `PRESENTATION`, and `elements()`.
    `PRESENTATION` is an ordinary `renderer::CollectionPresentation` value and cannot grant behavioral capabilities.
    ...
    `Vec`, `VecDeque`, and `LinkedList` (sequences), `BinaryHeap` (arbitrary bag), and `BTreeSet` and `HashSet`
    (sets).
  - `StableOrder: Collection` is the explicit capability for meaningful ordinal positions; `RandomAccess:
    StableOrder` adds constant-time `element_at`. `LinkedList` has stable order but not random access, while slices,
    arrays, `Vec`, and `VecDeque` have both. `StableOrderAssertions` (`collection/stable_order.rs`) owns all positional
    finite-collection assertions and requires `StableOrder`, so `contains_exactly`, `starts_with`, `ends_with`, and
    `contains_contiguous` on a set are compile errors rather than tests that pass for the wrong reason.
    `StableOrderExtractAssertions` in the same module adds `get_first`, `get_last`, and `get_single` in panic mode.
    `RandomAccessExtractAssertions` (`collection/random_access.rs`) adds `get_at`.
  - `SetLookup: Collection` (`set/mod.rs`) declares unique-element semantics and adds native membership lookup.
    `SetAssertions` requires it and holds the subset, superset, and disjointness relations, each accepting any other
    `SetLookup` so hashers and container types can differ.
  - `Map: HasLength` (`map/mod.rs`) carries `Key`, `Value`, presentation-only `RENDERING_ORDER`, and `entries()`. Lookup by key
  ...
  `#[cfg(feature = "std")]`. `CollectionPresentation` configures list/set syntax, type-hint visibility, and
  `RenderingOrder`; it is independent of `StableOrder`, `RandomAccess`, and `SetLookup`. Maps use the same
  `RenderingOrder`. Values configured as `SortByRenderedText` are sorted by rendered text and marked
  `(sorted for rendering)`.
  `renderer::GroupStyle` remains mandatory only for ad-hoc groups.
  `RenderingContext::collection` and `RenderingContext::map` apply these policies, while positional collection
  diagnostics use `RenderingContext::stable_collection` to preserve logical iteration order. The broad
  `IntoIteratorAssertions` trait remains order-free; direct `IteratorAssertions` treat yield order as meaningful.
  The assertion traits are re-exported by the prelude for method lookup. The implementor-facing `Collection`,
  `StableOrder`, `RandomAccess`, `SetLookup`, `Map`, `MapLookup`, and `MapKeyQuery` stay out of the prelude.
...
trait), order-sensitive ones into `StableOrderAssertions` via `StableOrder`, set relations into `SetAssertions` via
`SetLookup`,
...
`stable_order.rs`. Test each built-in adapter's `length()`, iteration/lookup, capabilities, presentation, and reference
...
5. On failure, call `self.fail(...)` with a formatted message, or `self.fail_with_details(details, ...)` when the
   ...
   Render every value the message shows through `self.render()`: `value` for a leaf, `variant` and `struct_field`
   for wrappers, `values` for an ad-hoc list, `collection` and `map` for a `Collection` or `Map` subject. This applies
   the active renderer and the `RenderingBudget`; never format a subject with `{:?}` directly
